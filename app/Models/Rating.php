...
class Rating extends AbstractModel
{
    ...
    public function save(): bool
    {
        ...
        if (!empty($this->id)) {
             ...
             */
            return $database->query("UPDATE $tablename SET user_id = ?, product_id = ?, rating = ?, title = ?, description = ? WHERE id = ?", [
             ...
             */
            $result = $database->query("INSERT INTO $tablename SET user_id = ?, product_id = ?, rating = ?, title = ?, description = ?", [
                'i:user_id' => $this->user_id,
                'i:product_id' => $this->product_id,
                's:rating' => $this->rating,
                's:title' => $this->title,
